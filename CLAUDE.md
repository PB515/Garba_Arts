...
- **Phase:** 2 of N — All v1 features built and verified live in the browser; real locations/batches seeded. Excel import is the only remaining planned feature, blocked on the owner sharing the file.
- **Last completed:** Full CRUD build (students/leads, payments, dashboard, CSV export), every flow tested live end-to-end, two real bugs found and fixed during that testing (FK constraint + audit-log ordering — see Phase 2 build log). Then seeded the real 2 locations (Aliya, Sportsclub) and 6 batches (8-9 PM / 9-10 PM / 10-11 PM at each) via a data migration, and fixed a real usability bug it immediately exposed: batch names repeat across locations, so every batch dropdown now shows `Location · Batch` instead of the ambiguous bare batch name.
- **Next up:** Nothing blocking — the app is functionally complete for v1 with real reference data. Remaining: (1) create real team accounts once names/emails are provided, (2) build the CSV import once the Excel file is shared, (3) confirm the starter status-tag list with the owner, (4) fill `globals.css` tokens with real brand colors before considering this "done" (currently the loud magenta placeholder — low priority for an internal tool per the Essential craft tier, but still required-not-optional per IDP convention), (5) deploy.
- **Last commit:** `721bb09` — full v1 feature build (the locations/batches seed + batch-label fix not yet committed as of this edit)
...
17. **Batch dropdowns must show `Location · Batch`, never the bare batch name** — confirmed live that identical batch names across locations (e.g. "8-9 PM" at both) make a bare-name dropdown genuinely ambiguous, not just cosmetically ugly. → `app/students/page.tsx`, `app/students/[id]/page.tsx`
