...
`locations`/`batches`/`navratri_registrations` are **not** location-scoped. `event_registrations`/`event_attendees` **are**, resolving the earlier open question — but on the registration, not on `events` itself, since an event's venue (it can happen anywhere, e.g. a party plot) is unrelated to Aalay/Sportsclub; what matters is which location each *registrant* came from. `event_registrations.location_id` is nullable so a public self-registration can stay unattributed — visible only to `super_admin`, invisible to every `location_admin`, counted in an event's combined total but not in any location's own breakdown.
...
4. **Joined tab** — everyone currently `status = joined`, with batch/location and a simple Paid / Not Paid / Half Paid badge (no rupee figures on this shared list — see "Data that must stay private"). A "Complete details" link appears next to anyone still missing batch or fee info. Pending fees are visible two ways: a light red/yellow tint on the whole row for Not Paid/Half Paid, and a "Pending fees only" checkbox filter — both in-app/visual, deliberately not an active notification system.
...
9. **CSV export** (super_admin only) — on Inquiry, Joined, and Fees, each carrying that page's own current filters into the download so what's exported matches what's on screen. Inquiry/Joined export a student roster (one row per student); Fees exports its payment log (one row per payment, since that page's filters — including payment mode — don't map onto a student-shaped row).
...
11. **Event registration, two paths** — staff logs a registration manually (name, phone, **location** — Aalay or Sportsclub, required — fee/paid, extra attendees), **or**, per-event opt-in (`public_registration_enabled`), a real public self-registration page at `/events/[id]/register` (no login) that anyone with the link can use. Extra attendees are entered via a number input ("how many additional people are coming?") that dynamically generates that many rows, each with Name (required), Phone, and WhatsApp (both optional) — capped at 20 rows. Both paths capture the same real per-attendee detail, not just a headcount — the owner's explicit ask, twice: first for names ("we don't just want number coming with student but also who are coming, number and name"), then for phone/WhatsApp too ("name and whatsapp number all we need to collect"). The event itself carries no location (its venue is irrelevant, e.g. a party plot); a `super_admin` sees a by-location breakdown of who registered from where, a `location_admin` only ever sees their own location's registrants. Public self-registrations have no location (the form doesn't ask) and are visible only to `super_admin`, counted in the combined total but not attributed to either location.
